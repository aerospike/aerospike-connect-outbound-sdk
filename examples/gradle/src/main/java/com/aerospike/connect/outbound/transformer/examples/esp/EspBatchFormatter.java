...
/**
 * EspBatchFormatter formats a batch of change notification records into a
 * single ESP message.
 *
 * <p>
 ...
 * format:
 *   mode: custom
 *   class: com.aerospike.connect.outbound.transformer.examples.esp.EspBatchFormatter
 * </pre>
 */
public class EspBatchFormatter implements BatchFormatter<EspOutboundMetadata> {
    private final static Logger logger =
            LoggerFactory.getLogger(EspBatchFormatter.class.getName());

    private final static ObjectMapper objectMapper = new ObjectMapper();

    @Override
    public List<OutboundRecord<EspOutboundMetadata>> format(
            @NonNull List<BatchItem<EspOutboundMetadata>> batchItems)
            throws Exception {
        ...
        // Metadata will be the same for all the records in the batch.
        EspOutboundMetadata metadata = batchItems.get(0).getFormattedRecord()
                .getMetadata();
